...
module Password_file = struct
  ...
  type t = { digit1 : int; digit2 : int; tchar : char; password : string }
...
module Part1 = struct
  let verify_password (p : Password_file.t) =
    let l =
      CCString.to_list p.password |> CCList.filter (fun c -> c = p.tchar)
    in
...
module Part2 = struct
  let verify_password (p : Password_file.t) =
    ...
    if CCList.length l < p.digit2 then false
    else
